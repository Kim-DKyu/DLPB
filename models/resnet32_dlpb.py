...
class ResNet(nn.Module):
    def __init__(self,
        ...
        for i in range(self.branches):
            self.gram_contraction.append(nn.Sequential(nn.Conv2d(64, self.gram_dim, kernel_size=1, stride=1,
                                                                 padding=0, bias=True, groups=1),
                                                       nn.BatchNorm2d(self.gram_dim)))

            if gram_layer:
                self.gram_layer.append(block(self.gram_dim, self.gram_dim, stride=1, norm_layer=norm_layer))

            else:
                self.gram_layer.append(nn.Identity())

            self.gram_embedding.append(nn.Sequential(nn.Conv2d(((self.gram_dim + 1) * self.gram_dim // 2), 64,
                                                               kernel_size=1, stride=1, padding=0, bias=True,
                                                               groups=self.gram_embedding_gropus), nn.BatchNorm2d(64)))
            self.ga.append(LayerScaleBlockClassAttn(64, num_heads=1, mlp_block_groups=1, dim_embed=self.dim_embed))
            self.fc.append(nn.Linear(64, num_classes))
...
model_config = {
    'resnet32_gram_dim_32_branch_3_dlpb': {'parameter': dict(nblock=[5, 5, 5, 1], block=BasicBlock, gram_dim=32, branches=3), 'etc': {}},
}
...
if __name__ == '__main__':
    import torch
    model_name = 'ga_resnet32_gram_dim_32_branch_4'
