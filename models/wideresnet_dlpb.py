...
class ResNet(nn.Module):
    def __init__(self,
        ...
        for i in range(self.branches):
            self.gram_contraction.append(nn.Sequential(nn.Conv2d(channels[-1]*block.factor, self.gram_dim, kernel_size=1, stride=1,
            ...
            self.gram_embedding.append(nn.Sequential(nn.Conv2d(((self.gram_dim + 1) * self.gram_dim // 2), channels[-1]*block.factor,
                                                               kernel_size=1, stride=1, padding=0, bias=True,
                                                               groups=self.gram_embedding_gropus), nn.BatchNorm2d(channels[-1]*block.factor)))
            self.ga.append(LayerScaleBlockClassAttn(channels[-1]*block.factor, num_heads=1, mlp_block_groups=1, dim_embed=self.dim_embed))
            self.fc.append(nn.Linear(channels[-1]*block.factor, num_classes))
    ...
    def make_layer(self, block, nblock: int, channels: int, stride: int, branch_ch: bool = False) -> nn.Sequential:
        if branch_ch:
            if nblock!=1:
                ...
                self.in_channels = self.agg_feat_dim # 448 112
    ...
    def forward(self, x):
        ...
        x = self.layer3(input)
        x_cat.append(x)
        x = torch.cat((self.avg_pool_down(x_cat[0]), x_cat[1], self.upsample_f(x_cat[2])),dim=1)
        x = self.layer4(x)
...
model_config = {
    'wide_resnet20_8_ga_64_branch_3_dlpb': {
        'parameter': dict(nblock=[2, 2, 2, 1], block=BottleNeck, base_width=64 * 8,
                          gram_dim=64, branches=3, agg_feat_dim=448),
        'etc': {},
    },
    'wide_resnet40_2_ga_64_branch_3_dlpb': {
        'parameter': dict(nblock=[6, 6, 6, 1], block=BottleNeck, base_width=64 * 2,
                          gram_dim=64, branches=3, agg_feat_dim=448),
        'etc': {},
    },
}
...
if __name__ == '__main__':
    import torch
    model_name = 'wide_resnet20_8_ga_64_branch_3'
