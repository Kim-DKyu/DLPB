...
try:
    from models.densenet_dlpb import create_densenet
except ImportError:
    from models.densenet_dlpb import create_densenet
...
def validate(valid_dataloader, model, criterion, args, mode='org'):
    ...
    top1_m_ens = Metric(reduce_every_n_step=args.print_freq, header='ens:')
    ...
    for batch_idx, (x, y) in enumerate(valid_dataloader):
        ...
        if batch_idx and args.print_freq and batch_idx % args.print_freq == 0:
            ...
            args.log(
                f"VALID({mode}): [{batch_idx:>{num_digits}}/{total_iter}] {batch_m} {data_m} {loss_m} {top1_m} {top5_m}")
    ...
    args.log('-' * space * num_metric)
    args.log(
        f"{'VALID(' + mode + ')':>{space}}{duration:>{space}}{data:>{space}}{f_b_o:>{space}}{top1:{space}.4f}{top5:{space}.4f}")
    args.log('-' * space * num_metric)

    return loss, top1, top5


def train_one_epoch(train_dataloader, model, optimizer, criterion, args, ema_model=None, scheduler=None, scaler=None,
                    epoch=None):
    # 1. create metric
    data_m = Metric(reduce_every_n_step=0, reduce_on_compute=False, header='Data:')
    batch_m = Metric(reduce_every_n_step=0, reduce_on_compute=False, header='Batch:')
    ...
    for batch_idx, (x, y) in enumerate(train_dataloader):
        ...
        with torch.cuda.amp.autocast(args.amp):
            ...
            loss += args.alpha * criterion(y_hat, y)
            ...
            for out_ in outputs:
                loss_distil_re = F.kl_div(F.log_softmax(y_hat / args.temperature), F.log_softmax(
                    out_.detach() / args.temperature), reduction='batchmean', log_target=True) * (
                                         args.temperature ** 2)
                loss += loss_distil_re

            loss_distil = F.kl_div(F.log_softmax(y_hat / args.temperature), F.log_softmax(
            ...
            for i, out in enumerate(outputs):
                loss_dec = F.kl_div(F.log_softmax(out + 0), F.log_softmax(
                    (output_add.detach() / len(outputs)) + 0), reduction='batchmean', log_target=True) * args.GA_lamb
                loss += loss_dec
    ...
    args.log('-' * space * num_metric)
    args.log(
        f"{'TRAIN(' + str(epoch) + ')':>{space}}{duration:>{space}}{data:>{space}}{f_b_o:>{space}}{loss:{space}.4f}")
    args.log('-' * space * num_metric)
...
def run(args):
    ...
    # 2. make model
    model = create_densenet(args.model_name, args.num_classes)
    ...
    for epoch in range(start_epoch, end_epoch):
        ...
        train_loss = train_one_epoch(train_dataloader, ddp_model if args.distributed else model, optimizer, criterion,
                                     args, ema_model, scheduler, scaler, epoch)
        val_loss, top1, top5 = validate(valid_dataloader, ddp_model if args.distributed else model, valid_criterion,
                                        args, 'org')
        if args.ema:
    ...
    if args.is_rank_zero:
        ...
        Result(args.output_dir).save_result(args, top1_list, top5_list,
                                            dict(duration=duration, best_acc=best_acc, avg_top1_acc=top1,
                                                 avg_top5_acc=top5))
